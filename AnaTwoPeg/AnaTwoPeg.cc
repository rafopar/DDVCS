...
int main(int argc, char** argv) {
    ...
    const int emPID = 11;
    const int pPID = 2212;
    const int pipPID = 211;
    const int pimPID = -211;
    ...
    const double ubn = 1.e-30;
    const double Qp2Cut_EPIC = 1.2; // GeV

    const double pion_cTau = 780; // cm

    const double pionDecayLengthCutoff = 90.; // The proper way would be to pass those pion to GEMC, but in this situation we will assume the pion is decayed
    // decayed to muon if it decayed prior to traveling pionDecayLengthCutoff distance
    ...
    TLorentzVector L_pim, L_pip; // We need pion real LorentzVectors too in order to make decay them properly
    ...
    //tr1->Add(Form("Data/twopeg_25Files.root/tr1", run));
    tr1->Add(Form("Data/twopeg_999Files.root/tr1", run));
    ...
    TFile file_out(Form("AnaTwoPeg_%d.root", run), "Recreate");
    TH1D h_Minv1("h_Minv1", "", 200, 0., 4.);
    TH1D h_Minv2("h_Minv2", "", 200, 0., 4.);
    TH1D h_Minv3("h_Minv3", "", 200, 0., 4.);
    ...
    for (int i_ev = 0; i_ev < nev; i_ev++) {
        ...
        bool em_MC_Read = false;
        bool p_MC_Read = false;
        bool pip_MC_Read = false;
        bool pim_MC_Read = false;

        double Etot = 0;
        for (int iPart = 0; iPart < nPart; iPart++) {
            if (pid[iPart] == emPID) {
                //cout<<iPart<<"  "<<px[iPart] <<"   "<< py[iPart] <<"   "<< pz[iPart] <<"   "<< E[iPart]<<endl;
                L_em.SetPxPyPzE(px[iPart], py[iPart], pz[iPart], E[iPart]);
                Etot = Etot + E[iPart];
                em_MC_Read = true;
            } else if (pid[iPart] == pPID) {
                L_prot.SetPxPyPzE(px[iPart], py[iPart], pz[iPart], E[iPart]);
                Etot = Etot + E[iPart];
                p_MC_Read = true;
            } else if (pid[iPart] == pipPID) {
                // Let's assign muon mass to pions, as those events will be considered as muons in the analysis
                double Emu = sqrt(px[iPart] * px[iPart] + py[iPart] * py[iPart] + pz[iPart] * pz[iPart] + Mmu * Mmu);
                L_pip.SetPxPyPzE(px[iPart], py[iPart], pz[iPart], E[iPart]);
                L_mup.SetPxPyPzE(px[iPart], py[iPart], pz[iPart], Emu);
                Etot = Etot + E[iPart];
                pip_MC_Read = true;
            } else if (pid[iPart] == pimPID) {
                // Let's assign muon mass to pions, as those events will be considered as muons in the analysis
                double Emu = sqrt(px[iPart] * px[iPart] + py[iPart] * py[iPart] + pz[iPart] * pz[iPart] + Mmu * Mmu);
                L_pim.SetPxPyPzE(px[iPart], py[iPart], pz[iPart], E[iPart]);
                L_mum.SetPxPyPzE(px[iPart], py[iPart], pz[iPart], Emu);
                Etot = Etot + E[iPart];
                pim_MC_Read = true;
            }
        }

        if (!(em_MC_Read && p_MC_Read && pip_MC_Read && pim_MC_Read)) {
            cout << "Not all FS particles are initialized, skipping this event." << endl;
            continue;
        }

        L_targ.SetPxPyPzE(0., 0., 0., Mp);

        L_beam.SetPxPyPzE(0., 0., Etot - Mp, Etot - Mp);
        ...
        bool pip_convert = rand->Exp(pion_cTau * L_pip.Gamma() * L_pip.Beta()) < pionDecayLengthCutoff;
        bool pim_convert = rand->Exp(pion_cTau * L_pim.Gamma() * L_pim.Beta()) < pionDecayLengthCutoff;
        ...
        if (em_Acc && mum_Acc && mup_Acc /* && prot_Acc */) {
            ...
            h_Minv2.Fill(L_mummup.M(), EvWeight);

            if (pip_convert && pim_convert) {
                h_Minv3.Fill(L_mummup.M(), EvWeight);
            }
    ...
    double weight = totLumi * ubn / double(nev);
