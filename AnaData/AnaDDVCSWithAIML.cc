...
int main(int argc, char **argv) {
    ...
    */
    ...
    const double MinvMinCut = 1.5; // To consider for DDVCS analysis, the Minv should be above this cut.
    ...
    std::vector<double> mean = j["mean"];
    std::vector<double> stdv = j["std"];


    std::cout<<"size of mean is "<<mean.size()<<std::endl;
    std::cout<<"size of stdv is "<<stdv.size()<<std::endl;
    ...
    try {
        while (reader.next() == true) {
            ...
            if (n_em == 2 && n_ep == 1) {
                ...
                torch::Tensor input = torch::tensor(xx).reshape({1, (int)xx.size()});
                ...
                if (TMath::Abs(part_em1.vt() - v_t_em2) < v_dtCut) {
                    ...
                    if (m_emep1 > 0.25 && m_emep2 > 0.25) {
                        ...
                        if ( MxRecoil > MxRecoil_Min && MxRecoil < MxRecoil_Max ) {
                            ...
                            h_Qp2_Q2_1_3.Fill(Q2_1_NoProt, Qp2_1_NoProt);
...
double Minv2Part( const RecParticle& p1, const RecParticle& p2 ) {
    double Minv = sqrt( (p1.p() + p2.p())*(p1.p() + p2.p()) - (p1.px() + p2.px())*(p1.px() + p2.px()) - (p1.py() + p2.py())*(p1.py() + p2.py()) - (p1.pz() + p2.pz())*(p1.pz() + p2.pz()) );
    return Minv;
}