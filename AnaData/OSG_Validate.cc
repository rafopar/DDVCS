 ...
 * 
 */
void OSG_Validate(int job1, int job2, int job3) {
    
    cout<<"job1 is "<<job1<<"  is the OSG Job with the current portal."<<endl;
    cout<<"job2 is "<<job2<<"  is the OSG Job with the New portal and New Software."<<endl;
    cout<<"job3 is "<<job2<<"  is the OSG Job with the New portal and Old (pass1) Software."<<endl;

    TLatex *lat1 = new TLatex();
    lat1->SetNDC();
    lat1->SetTextFont(42);

//    TFile *file_CurPortal = new TFile("Hists_DDVCS_GRAPE_Run6_CurrentPortal.root", "Read");
//    TFile *file_NewPortal = new TFile("Hists_DDVCS_GRAPE_Run6_NewPortal.root", "Read");
//    TFile *file_NewPortalOldSW = new TFile("Hists_DDVCS_GRAPE_Run6_NewPortalOldSW.root", "Read");
    ...
    lat1->DrawLatex(0.55, 0.7, "Current OSG");
    lat1->SetTextColor(h_MC_emep_pair2_NewPortalOldSW->GetLineColor());
    lat1->DrawLatex(0.55, 0.65, "Test OSG: gemc/4.4.2 coatjava/6.5.6.1");
    ...
    lat1->DrawLatex(0.35, 0.7, "Current OSG");
    lat1->SetTextColor(h_Mmis1_NewPortal_OldSW->GetLineColor());
    lat1->DrawLatex(0.35, 0.65, "Test OSG: gemc/4.4.2 coatjava/6.5.6.1");
